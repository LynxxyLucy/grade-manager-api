...
import repo from "../repositories/subjectRepository.js";
...
router.get("/", async (req, res) => {
  ...
  try {
    const subjects = await repo.findMany({ semesterId });
...
router.get("/:id", async (req, res) => {
  ...
  try {
    const subject = await repo.findUniqueWithGrades({ id });
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    const deletedSubject = await repo.deleteMany({ id });
